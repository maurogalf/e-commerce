...
router.post('/form/newproduct', (req, res) => {
    if (req.query.admin === 'true') {
        productosDao.getAll().then((data) => {
            ...
            newProduct.id = data.length == 0 ? 1 : data.length + 1;
            newProduct.timestamp = new Date();
            productosDao.newOne(newProduct).then(() => {
...
router.get('/:id', (req, res) => {
    productosDao.getById(req.params.id).then((data) => {
        let product = []
        product.push(data)
        if (req.query.admin === 'true') {
            res.render('products', { products: product, admin: true, id: true });
        } else {
            res.render('products', { products: product, admin: false, id: true });
        }
    }).catch((err) => {
...
router.put('/:id', (req, res) => {
    if (req.query.admin === 'true') {
        productosDao.getAll().then((data) => {
            ...
            productosDao.update(productUpdated).then(() => {
                res.send({ error: 0, description: 'Producto actualizado correctamente' });
            }).catch((err) => {
                res.send({ error: err, description: 'Error al actualizar el producto' });
            });
        }).catch((err) => {
            res.send({ error: err, description: 'Error al leer la coleccion' });
        });
    } else {
        res.send({ description: 'No tienes permisos para acceder a esta ruta' });
    }
});
